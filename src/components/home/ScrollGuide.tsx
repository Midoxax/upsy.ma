...
const ScrollGuide = memo(({ message, position = "right", variant = "wave" }: ScrollGuideProps) => {
  ...
  // Mascot SVG — a friendly neural node face
  const mascotSize = 64;
  ...
  return (
    <motion.div
      className={`flex items-end gap-3 py-6 px-4 md:px-8 max-w-md ${isLeft ? "mr-auto" : "ml-auto flex-row-reverse"}`}
    ...
    >
      ...
      >
        <svg width={mascotSize} height={mascotSize} viewBox="0 0 76 76" fill="none">
          {/* Body — neural orb */}
      ...
      {/* Speech bubble */}
      <motion.div
        className={`relative rounded-2xl px-5 py-3 text-sm font-medium text-foreground/80 bg-card/80 backdrop-blur-sm border border-border/50 shadow-sm ${
